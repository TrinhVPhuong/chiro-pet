...
- **[priority=highest, scope=universal, trigger=all_questions]** :: !!! The Verification Before Response Protocol (Anti-Hallucination) !!! :: Whenever the user asks a question, requests a feature, or provides files for context, you MUST NOT generate a direct answer or solution from memory. You MUST open a `<thinking>` block to analyze: "Do I have 100% of the actual data from the project files to answer this?". If NO or UNSURE, you MUST use tools like `read_file`, `search_files`, or `sequentialthinking` to gather facts. You are STRICTLY FORBIDDEN to use memory-based assumptions to guess code or logic.
- **[priority=critical, scope=universal, trigger=brainstorming]** :: !!! Brainstorming Trigger Guardrail !!! :: You MUST invoke the `brainstorming` skill ONLY when tasked with creating ENTIRELY NEW features, modules, or systems that DO NOT have an existing plan or specification file. You are STRICTLY FORBIDDEN to call the `brainstorming` skill when debugging, understanding existing code, answering localized questions, or when a plan/spec file already exists.
