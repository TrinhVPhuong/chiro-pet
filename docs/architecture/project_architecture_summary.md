...
- **[`useVRMScene`](../../src/hooks/useVRMScene.ts)**: 
  ...
  - **Memory Leak Disposal**: Giải phóng GPU memory triệt để khi unmount bằng cách duyệt qua toàn bộ mesh, geometry, material và texture để `.dispose()` thủ thủ công.
- **[`useAltKeyTracking`](../../src/hooks/useAltKeyTracking.ts)**: Hook chuyên biệt lắng nghe và cập nhật trạng thái phím ALT thông qua Event API của Tauri.
- **[`useDrag`](../../src/hooks/useDrag.ts)**: Chuyển đổi tọa độ dịch chuyển của chuột (Screen Space) sang tọa độ thế giới 3D (World Space) của OrthographicCamera một cách chính xác.

### 1.2 Layout & Presentational Components
- **[`CharacterCanvas`](../../src/components/CharacterCanvas.tsx)**: Nơi mount canvas của WebGLRenderer. Được set `pointer-events-none` để người dùng click xuyên qua khu vực trong suốt của nhân vật xuống màn hình desktop.
- **[`DragOverlay`](../../src/components/DragOverlay.tsx)**: Một overlay bao phủ toàn bộ cửa sổ. 
  > [!NOTE]
...
- **[`RadialMenu`](../../src/components/RadialMenu.tsx)**: Context menu chuột phải được nâng cấp các tính năng UX cao cấp:
...
- **[`StatusIndicator`](../../src/components/StatusIndicator.tsx)** & **[`LoadingIndicator`](../../src/components/LoadingIndicator.tsx)**: Các UI bổ trợ hiển thị trạng thái kéo thả và tiến trình load nhân vật.
...
- **[`commands.rs`](../../src-tauri/src/commands.rs)**: Chứa các lệnh IPC (Inter-Process Communication) phơi bày cho Frontend gọi qua `invoke`. Lệnh `greet` thừa của template ban đầu đã bị loại bỏ.
- **[`input_tracking.rs`](../../src-tauri/src/input_tracking.rs)**: Trình theo dõi trạng thái phím ALT và chuột ở mức toàn cục (Global) bằng tiến trình nền (background task).
...
- **[`lib.rs`](../../src-tauri/src/lib.rs)**: Đóng vai trò là Composition Root cực kỳ mỏng, chỉ thực hiện setup Tauri builder, plugin và chạy task tracking.
...
## 🎬 5. Hệ thống Animation Runtime (Mới)

### 5.1 Kiến trúc cốt lõi
Hệ thống được thiết kế theo nguyên tắc **Backend-driven State**, nghĩa là Rust đóng vai trò "Source of Truth" quyết định khi nào, chạy hoạt ảnh gì. Frontend chỉ làm nhiệm vụ "View" để render mượt mà.

- **Animation Director (Rust)**: Orchestrator quản lý toàn bộ các yêu cầu hoạt ảnh đến từ nhiều nguồn (AI chat, User drag, Background task).
- **Context Registry (Rust)**: Hỗ trợ cơ chế xếp hàng (queue) và quyền ưu tiên (priority). Ví dụ: Đang nói chuyện (priority 60) nhưng bị kéo thả chuột (priority 95) sẽ bị ngắt, sau khi thả chuột, sẽ quay về nhàn rỗi (idle) hoặc tiếp tục hành động cũ.
- **IPC Command**: Giao tiếp sử dụng struct `AnimationCommand` chuẩn hóa qua Tauri Emitter.
- **Manifest (`manifest.json`)**: File config tại Frontend định nghĩa tham số cho từng file VRMA (thời gian crossfade, vòng lặp, tag, priority mặc định).

### 5.2 Xử lý Blending tại Client (Three.js)
Trong module `AnimationController`, áp dụng kỹ thuật **4-Layer Blend** tiên tiến:
1. **Layer 1 (Base)**: Vòng lặp Idle mặc định (weight = 1.0).
2. **Layer 2 (Action)**: Cử chỉ đè lên (Action clips) như vẫy tay, nói chuyện. Hỗ trợ crossfade mượt mà. Lớp này sử dụng `SectionedPlayback` để chạy thông minh các clip có dạng intro-loop-outro.
3. **Layer 3 (Procedural)**: Cử động thở (breathing), nhấp nháy, lắc lư dựa trên hàm toán học, giúp nhân vật không bao giờ bị đơ cứng. Tự động giảm cường độ khi Layer 2 hoạt động mạnh.
4. **Layer 4 (Expression)**: Hệ thống blendshape cho nét mặt (vui, buồn, suy nghĩ). Tách biệt hoàn toàn khỏi xương cơ thể.
